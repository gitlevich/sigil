...
A desktop tool for writing hierarchical specifications before you write code.

## Who this is for

You're a software architect, technical lead, or senior engineer who believes that unclear specifications produce unclear systems. You've seen projects where the team started coding before anyone agreed on what the system actually is — and you've watched the slow, expensive reckoning that follows.
...
Sigil is a specification editor organized as a tree of bounded contexts. Each context has a name, a spec body written in markdown, optional technical decisions, and up to five sub-contexts. The hard limit of five is the point. It enforces the cognitive discipline that keeps specifications coherent as they grow.

The spec tree lives on your file system as a directory hierarchy — plain directories and markdown files. No proprietary format, no database, no lock-in. Put it in git and it versions like code.

**Core features:**

- **Hierarchical editing** — navigate a tree of contexts, each with its own spec body and technical decisions. Technical decisions inherit downward unless overridden.
- **Vision statement** — a persistent reminder of what the application is for, visible from any depth in the tree. Prevents drift.
- **AI review** — a built-in chat panel where an AI agent reviews your entire spec tree for coherence, gaps, contradictions, and implementation readiness. Supports Anthropic and OpenAI.
- **Multi-window** — each spec tree opens in its own native window. Compare two specs side by side.
