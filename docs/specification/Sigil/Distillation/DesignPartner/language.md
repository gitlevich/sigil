...
Teaching the partner by talking to it writes the sigil. The conversation that establishes a domain concept, names a signal, declares an affordance — that conversation is the Edit that grows the trajectory. The sigil accumulates the memory of every session. Re-loading the sigil re-inhabits the accumulated memory.
