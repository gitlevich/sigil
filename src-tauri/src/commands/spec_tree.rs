use std::fs;
use std::path::Path;
use crate::models::spec_tree::{Context, SpecTree};
...
fn read_context(dir: &Path) -> Result<Context, String> {
    ...
    if let Ok(entries) = fs::read_dir(dir) {
        ...
        for entry in dirs {
            ...
            if child_path.join("spec.md").exists() {
...
#[tauri::command]
pub fn read_spec_tree(root_path: String) -> Result<SpecTree, String> {
...
pub fn create_context(parent_path: String, name: String) -> Result<Context, String> {
    ...
    fs::write(context_path.join("spec.md"), "").map_err(|e| e.to_string())?;
...
mod tests {
    ...
    fn setup_spec_tree(tmp: &TempDir) -> String {
        let root = tmp.path().join("MyApp");
        fs::create_dir(&root).unwrap();
        fs::write(root.join("vision.md"), "Build the best app").unwrap();
        fs::write(root.join("spec.md"), "# MyApp\nRoot spec").unwrap();
        fs::write(root.join("technical.md"), "Use Rust").unwrap();

        let auth = root.join("Auth");
        fs::create_dir(&auth).unwrap();
        fs::write(auth.join("spec.md"), "Auth handles login").unwrap();

        let billing = root.join("Billing");
        fs::create_dir(&billing).unwrap();
        fs::write(billing.join("spec.md"), "Billing handles payments").unwrap();

        root.to_string_lossy().to_string()
    }

    #[test]
    fn test_read_spec_tree() {
        let tmp = TempDir::new().unwrap();
        let root_path = setup_spec_tree(&tmp);

        let tree = read_spec_tree(root_path).unwrap();

        assert_eq!(tree.name, "MyApp");
        assert_eq!(tree.vision, "Build the best app");
        assert_eq!(tree.root.spec_body, "# MyApp\nRoot spec");
        assert_eq!(tree.root.technical_decisions, Some("Use Rust".to_string()));
    ...
    #[test]
    fn test_read_spec_tree_nonexistent() {
        let result = read_spec_tree("/nonexistent/path".to_string());
    ...
    fn test_create_context() {
        ...
        assert!(ctx.spec_body.is_empty());
        assert!(ctx.children.is_empty());

        // Verify file was created
        let spec_path = Path::new(&root_path).join("Notifications/spec.md");
    ...
    fn test_create_context_max_five() {
        ...
        // Already has Auth and Billing (2), add 3 more to reach 5
    ...
    fn test_rename_context() {
        ...
        assert!(Path::new(&new_path).join("spec.md").exists());
    }
    ...
    #[test]
    fn test_read_tree_ignores_hidden_dirs() {
        let tmp = TempDir::new().unwrap();
        let root_path = setup_spec_tree(&tmp);

        let hidden = Path::new(&root_path).join(".git");
        fs::create_dir(&hidden).unwrap();
        fs::write(hidden.join("spec.md"), "should be ignored").unwrap();
    ...
    #[test]
    fn test_read_tree_ignores_dirs_without_spec() {
        let tmp = TempDir::new().unwrap();
        let root_path = setup_spec_tree(&tmp);
