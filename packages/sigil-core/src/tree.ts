...
export function findContext(root: Sigil, path: string[]): Sigil {
  let current = root;
...
/** Find path from root to a sigil by name (DFS, case-insensitive). */
...
/** Extract a short summary from language (strips frontmatter, takes first 3 non-heading lines). */
export function makeSummary(sigil: Sigil): string {
  let text = sigil.language || "";
  if (text.startsWith("---")) {
